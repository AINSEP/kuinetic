...
- **A bundle carries `on:`, `timeline:`, `threshold:`, `rm:` and `func:`** to its usage site,
  ...
  element writes itself always wins; two bundles that disagree warn. The group keys
  (`cascade:`/`spread:`/`order:`/`cols:`/`along:`) are the exception and are refused with a warning, because `stagger.ts` reads them off the group element's
...
**Do not create one runtime chunk per name.** 292 named effects come from 34 primitive families — ship a compressed
...
That's why the catalog can carry 292 named effects from only 34 primitive families.
