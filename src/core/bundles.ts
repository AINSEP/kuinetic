...
/**
 * Element-scoped keys a bundle carries to its usage site. `cascade`/`spread`/`order` are excluded —
 * see `readBundle`.
 */
...
function readBundle(name: string, source: string, el: Element, reporter: Reporter): Bundle {
  ...
  // `cascade:`/`spread:`/`order:` are the hoists a bundle cannot carry, and the reason is a second
  ...
  const group = (['cascade', 'spread', 'order'] as const)
