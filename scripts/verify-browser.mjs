 ...
 * user's is touched.
 *
 * Every check but `checkShowcaseReplay` runs against `test/browser/fixtures/verify-browser.html`,
 * a page purpose-built for this suite — not a demo/showcase page. This suite used to point at
 * `demo/index.html` directly, which meant every visual redesign of that page was free to silently
 * disarm it; it did, when a redesign dropped `#onload` and friends with no test failure until
 * someone happened to run this script by hand. `checkShowcaseReplay` is the one deliberate
 * exception: it asserts the real, shipped `demo/reveals.html` still works end-to-end (its own
 * doc comment below explains why an id-based selector there is necessary, not incidental).
 *
 * Each `check*` function below covers one independent slice of the fixture (composition, stagger,
 * pinning, FLIP, ...) and is kept short and flat on purpose — this file used to be one 200+ line
 * `run()`, which was hard to scan and tripped the same complexity/length gate this repo enforces
 * everywhere else. Splitting along the existing comment banners groups each function's checks the
 * same way `it()` blocks group a test file's assertions.
 */
import { rmSync } from 'node:fs'
import { fileURLToPath } from 'node:url'
import { burstSample, createChecker, createFrameRecorder, loadChromium } from './browser-harness.mjs'

const PAGE_URL = `file://${fileURLToPath(new URL('../test/browser/fixtures/verify-browser.html', import.meta.url))}`
const SHOWCASE_URL = `file://${fileURLToPath(
  new URL('../demo/reveals.html', import.meta.url),
)}`
...
async function checkBoot(page, snap, consoleErrors) {
...
async function checkRevealHeldBeforeActivation(page, snap) {
  ...
  check(
    ...
    (await page.$eval('#reveal', (el) => el.getAttribute('data-kui-state'))) === 'ready',
  )
  await snap(page, 'reveal-held-before-activation')
}

async function checkRevealAfterScrollIntoView(page, snap) {
  ...
  check(
    ...
    (await page.$eval('#reveal', (el) => el.getAttribute('data-kui-state'))) === 'finished',
  )
  await snap(page, 'reveal-after-scroll-into-view')
}

async function checkComposition(page, snap) {
  ...
  check(
    ...
    composed.includes('kui-in-up') && composed.includes('kui-blur-in'),
    composed.join(', '),
  )
  ...
  check(
    ...
    combo.includes('kui-in-up') && combo.includes('kui-blur-in'),
    combo.join(', '),
  )
  await snap(page, 'combo-not-substituted')
}

async function checkParameters(page, snap) {
  check(
    'author parameter overrides the preset default',
    (await page.$eval('#override', (el) => el.style.getPropertyValue('--kui-distance'))) === '80px',
  )
  check(
    'rejected parameter is never written to the element',
    (await page.$eval('#rejected', (el) => el.style.getPropertyValue('--kui-distance'))) === '',
  )
  await snap(page, 'parameter-override-and-rejection')
}

async function checkStagger(page, snap) {
...
async function checkHover(page, snap) {
  check(
    ...
    (await page.$eval('#hoverable', (el) => getComputedStyle(el).animationPlayState)) === 'paused' &&
      (await page.$eval('#hoverable', (el) => el.getAttribute('data-kui-state'))) === 'ready',
  )
  await snap(page, 'hover-before-interaction')
  await page.hover('#hoverable')
  await page.waitForTimeout(300)
  const hoverState = await page.$eval('#hoverable', (el) => el.getAttribute('data-kui-state'))
  check(
...
async function checkPinning(page, snap) {
  check(
  ...
  check(
    ...
    (await page.$eval('#pin-host [data-kui-spacer]', (el) => el.getAttribute('aria-hidden'))) ===
      'true',
  )
...
async function checkFlip(page, snap) {
...
async function checkSvgMorph(page, snap) {
...
/**
 * The replay page itself matters here: calling `play()` in isolation would not prove the shared
 * FAB is wired through the public reset/replay path. Selected by id rather than by trigger,
 * because `play()` stamps `data-kui-on="manual"` on any element that lacks the longhand
 * attribute — which is every element on the page now that triggers are authored inline as
 * `on:load`. A selector keyed on the trigger would be unstable across the very click being
 * tested; an id is not.
 */
async function checkShowcaseReplay(context, snap) {
  const showcase = await context.newPage()
  await showcase.goto(SHOWCASE_URL)
  await showcase.waitForFunction(() => window.__kui !== undefined)
  ...
  // `currentTime` on the Web Animations API is measured from the start of the *delay* phase, not
  // the visible portion — an animation with any authored/staggered delay before it never reaches
  // `currentTime === duration`; it finishes at `delay + duration`. Reading both off the live
  // animation (rather than hardcoding the `600ms` the element's `data-kui` duration token names)
  // keeps this resilient to the showcase tuning its own load-in delay independently of this test.
  const totalMs = await loadEffect.evaluate((el) => {
    const timing = el.getAnimations()[0]?.effect?.getTiming()
    const duration = typeof timing?.duration === 'number' ? timing.duration : 600
    const delay = typeof timing?.delay === 'number' ? timing.delay : 0
    return duration + delay
  })

  await showcase.click('.kui-replay-fab')
  // A single before/after pair cannot tell a genuine restart from a frozen mid-range opacity
  // reading — burst-sample `currentTime`/`playState` across the whole duration instead, the same
  // way flip-geometry proves motion is real rather than just starting and ending right.
...
async function openPage(browser) {
  if (RECORD) rmSync(ARTIFACT_DIR, { recursive: true, force: true })
  ...
  return { context, page, consoleErrors, snap }
}
...
function reportResults() {
  ...
  const lines = failed.map((f) => `  - ${f.name} ${f.detail}`)
  console.log(`\nFAILED:\n${lines.join('\n')}`)
  process.exit(1)
}
...
async function run() {
  ...
  const { context, page, consoleErrors, snap } = await openPage(browser)

  await checkBoot(page, snap, consoleErrors)
  await checkOnLoad(page, snap)
  await checkRevealHeldBeforeActivation(page, snap)
  await checkRevealAfterScrollIntoView(page, snap)
  await checkComposition(page, snap)
  await checkParameters(page, snap)
  await checkStagger(page, snap)
  await checkHover(page, snap)
  await checkPinning(page, snap)
  await checkScrollytelling(page, snap)
  await checkFlip(page, snap)
  await checkSvgMorph(page, snap)
  await checkShowcaseReplay(context, snap)
  await checkReducedMotion(browser, snap)
